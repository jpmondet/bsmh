...
def create_playlist(maps, playlisttitle, last_hours, mapnumber, playlistoutputdir):
    if playlisttitle:
        ...
        PLAYLIST["playlistTitle"] = (
            f"last_{mapnumber}_maps" if mapnumber else f"last_{last_hours}h_maps"
        )
...
def download_songs(playlist_filename, output_directory="."):
    ...
    for i, bsmap in enumerate(PLAYLIST["songs"]):
        ...
        if not file_to_dl.exists():
            ...
            try:
                with get(
                    f"{BASE_URL}{DOWNLOAD_HASH}{bsmap['hash']}", headers=FAKE_HEADERS, stream=True
                ) as r:
                    with open(f"{zip_file}", "wb") as f:
                        copyfileobj(r.raw, f)
            except:
                print(
                    f"Oops, can't download {bsmap['songName']} :(  (hash: {bsmap['hash']}, key: {bsmap['key']}) Is it deleted?"
                )
                continue
        else:
        ...
        with open(playlist) as plist_file:
            ...
            for bsmap in plist["songs"]:
                ...
                try:
                    ...
                    try:
                        print(
                            f"infos from map {bsmap['hash']} are missing. Trying to get them from bsaver"
                        )
                        remote_map = get_map(bsmap["hash"])
                        songname = f"{remote_map['key']} ({remote_map['name']} - {remote_map['metadata']['levelAuthorName']})"
                    except KeyError:
                        print(
                            f"Damned, we can't remove the map {bsmap}: not enough info in playlist and it looks like it has been removed from bsaver :("
                        )
                        continue
                songname_file = "".join(char for char in songname if not char in ILLEGAL_CHARS)
...
def main():
    ...
    if maps:
        playlist_filename = create_playlist(
            maps, args.playlisttitle, args.last, args.mapnumber, args.playlistoutputdir
        )
        if args.download:
